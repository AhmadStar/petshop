...
<div class="card">
    <h5 class="card-header">Kullanıcıyı Düzenle</h5>
    <div class="card-body">
      <form method="post" action="{{route('users.update',$user->id)}}">
        @csrf 
        @method('PATCH')
        <div class="form-group">
          <label for="inputTitle" class="col-form-label">Ad</label>
        ...
        <div class="form-group">
            ...
            <select name="role" class="form-control">
                ...
                @foreach($roles as $role)
                    ...
                    <option value="{{$role->role}}" {{(($role->role=='user') ? 'selected' : '')}}>Kullanıcı</option>
                @endforeach
            </select>
          @error('role')
          <span class="text-danger">{{$message}}</span>
          ...
          <div class="form-group">
            <label for="status" class="col-form-label">Statü</label>
            <select name="status" class="form-control">
                <option value="active" {{(($user->status=='active') ? 'selected' : '')}}>Aktif</option>
                <option value="inactive" {{(($user->status=='inactive') ? 'selected' : '')}}>Inaktif</option>
            </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
